...
def bind_model(model, optimizer=None):
    def save(path, *args, **kwargs):
        joblib.dump(model, os.path.join(path, 'lgb.pkl'))
        print('Model saved')

    def load(path, *args, **kwargs):
        model = joblib.load(os.path.join(path, 'lgb.pkl'))
        print('Model loaded')
...
def inference(path, model, **kwargs):
    data = preproc_data(pd.read_csv(path))
...
def get_clf_eval(y_val, pred=None, pred_proba=None):
    ...
    print(f'Accuracy: {accuracy:.3f}, Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1:.3f}, AUROC:{roc_auc:.3f}')
